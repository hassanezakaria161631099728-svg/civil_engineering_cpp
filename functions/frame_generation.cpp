...
std::vector<Node> wall_bracing_Nodes(const frame_generation_input& d, double h, int n, double L)
{
    ...
    {
        double xCoord = (col == 0) ? 0.0 : d.L;

        for(int i = 0; i <= d.n; ++i)
        {
            Node node;
            node.id = col*(d.n+1) + i + 1;
            node.x  = xCoord;
            node.y  = i * d.h;
                                      ...
                                      double Ahor, double Ihor, double Adiag, double Idiag, double q)
{
    ...
    for(int i = 1; i <= d.n; ++i)
    {
        int leftBottom  = i;
        int leftTop     = i + 1;
        int rightBottom = d.n + 1 + i;
        int rightTop    = d.n + 1 + i + 1;
...
double Ahor, double Ihor, double Adiag, double Idiag, double q)
{
    ...
    for(int i = 0; i < d.n; i++)
    {
        ...
        // Bottom chord
        elements.push_back({id++, b1, b2, d.E, d.Ahor, d.Ihor, d.q, "local","horizontal"});

        // Top chord
        elements.push_back({id++, t1, t2, d.E, d.Ahor, d.Ihor, d.q, "local","horizontal"});

        // Vertical
        elements.push_back({id++, b1, t1, d.E, d.Aver, d.Iver, d.q, "local","vertical"});

        // Diagonal 1
        elements.push_back({id++, b1, t2, d.E, d.Adiag, d.Idiag, d.q, "local","diagonal"});

        // Diagonal 2
        elements.push_back({id++, t1, b2, d.E, d.Adiag, d.Idiag, d.q, "local","diagonal"});
    }
  // final right vertical
  int b_last = d.n+1;
  int t_last = bottomCount + b_last;
        elements.push_back({id++, b_last, t_last, d.E, d.Aver, d.Iver, d.q, "local","vertical"});
...
std::vector<DOF> roof_bracing_DOFs(const frame_generation_input& d, int n)
{
    ...
    int totalNodes = 2 * (d.n + 1);

    int leftSupportNode  = 1;
    int rightSupportNode = d.n + 1;
    int id = 1;
                   ...
                   int windowHeight)
{
    ...
    writeimagefile("elevation_view_YZ.bmp",
                   0, 0, getmaxx(), getmaxy());
...
std::vector<Node> tall_building_Nodes(const frame_generation_input& d,
    int nspan,
    int Lspan,
    int nstories,
    int h)
{
    ...
    for(int j = 0; j <= d.nstories; j++)        // stories (vertical)
    {
        for(int i = 0; i <= d.nspan; i++)      // spans (horizontal)
        {
            double x = i * d.Lspan;
            double y = j * d.h;
...
std::vector<Element> tall_building_Elements(const frame_generation_input& d,
    int nspan,
    int nstories,
    double E,
    ...
    double q)
{
    ...
    // -----------------------------------
    for(int j = 1; j <= d.nstories; j++)   // start from 1
    {
        for(int i = 0; i < d.nspan; i++)
        {
            int n1 = j*nx + i + 1;
            int n2 = n1 + 1;
